...
import { findUpcomingCourses } from "./findUpcomingCourses.course.mongo.js";
import { UserRole } from "../../utils/enums/userRole.js";

describe("Buscar cursos próximos con Mongo", () => {
    ...
    it("Debería devolver un array vacío si no hay cursos próximos", async () => {
        const courses = await findUpcomingCourses();
        expect(courses).toHaveLength(0);
    });

    it("Debería devolver solo cursos cuya startDate sea mayor o igual a hoy", async () => {
        ...
        await saveCourse({
            name: "Curso Próximo",
            description: "Curso que aún no comenzó",
            durationMonths: 3,
        ...
        await saveCourse({
            name: "Curso Pasado",
            description: "Curso que ya comenzó",
            durationMonths: 3,
        ...
        const upcomingCourses = await findUpcomingCourses();

        expect(upcomingCourses).toHaveLength(1);
        expect(upcomingCourses[0]?.name).toBe("Curso Próximo");
    });
});
