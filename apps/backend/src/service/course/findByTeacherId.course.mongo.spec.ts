...
import { findByTeacherId } from "./findByTeacherId.course.mongo.js";
import { UserRole } from "../../utils/enums/userRole.js";

describe("Buscar cursos por teacherId con Mongo", () => {
    ...
    it("Debería devolver un array vacío si no hay coincidencias", async () => {
        const courses = await findByTeacherId(new mongoose.Types.ObjectId().toString());
        expect(courses).toHaveLength(0);
    });

    it("Debería encontrar cursos por teacherId", async () => {
        ...
        const foundCourses = (await findByTeacherId(teacher._id.toString())) ?? [];
